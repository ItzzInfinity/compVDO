...
> Status lives in `roadmap.md` (aliased as `ToDo.md`), never in code.
...
that it will usually make the file bigger. See `requirements.md` §3 for the
quality ladder.
...
`requirements.md` precisely so both implementations can be checked against
one document.

## Product rules
Numbered and testable — see `requirements.md`. Summary:
...
## Docs map
`requirements.md` · `architecture.md` (→ `Architecture.md`) ·
`data-model.md` · `roadmap.md` (→ `ToDo.md`) ·
`checkpoints.md` (→ `memory.md`) · `qa-checklist.md` ·
